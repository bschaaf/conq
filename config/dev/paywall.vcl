 ...
 * the local authorization setup as can be found in
 * com.atex.gong.paywall.PaywallFilter which also contains setting of the headers
 * used here.
 *
 * --------------------------------------------------------------------------------
...
C{
    ...
    void grant_metered_access(void* sp, const char* articles, const char* timestamp, const char* limit_string, const char* requested_article) {
        long period = strtol(VRT_GetHdr(sp, HDR_RESP, "\021X-Metered-Period:"), NULL, 10);
        long period_seconds = 24 * 60 * 60 * period;
        char creation_time_str[40];
        if(timestamp == NULL || strncmp(timestamp, "", 1) == 0) {
            sprintf(creation_time_str, "%ld", (long) time(NULL));
            timestamp = creation_time_str;
        }

        long creation_time = strtol(timestamp, NULL, 10);
        char expiry_time_formatted[40];
        TIM_format(creation_time + period_seconds, expiry_time_formatted);
        VRT_SetHdr(sp, HDR_RESP, "\014X-Timestamp:", expiry_time_formatted, vrt_magic_string_end);
...
sub strip_headers {
    ...
    unset resp.http.X-Paywall-User-Products-Decoded;
...
sub prepare_paywall_headers {
    ...
    set resp.http.X-Paywall-User-Products-Decoded = digest.base64_decode(regsub(resp.http.X-Paywall-User-Products, ";.*", ""));

    set resp.http.X-User-Session-Key = regsub(req.http.Cookie, {"^.*accessToken="?"}, "");
    ...
    set resp.http.X-Crypto-Message = resp.http.X-Paywall-User-Products + ":" + resp.http.X-User-Session-Key;
    set resp.http.X-Calculated-Digest = digest.hmac_sha256("mysecret", resp.http.X-Crypto-Message);

    if ("0x" + resp.http.X-User-Digest == resp.http.X-Calculated-Digest) {
...
sub hard_paywall {
    if(resp.http.X-User-Valid-Digest == "true") {
        C{
            char *user_products = VRT_GetHdr(sp, HDR_RESP, "\040X-Paywall-User-Products-Decoded:");
...
sub metered_paywall {
    if(req.http.Cookie ~ "p_metered_access") {
        ...
        set resp.http.X-Metered-Cookie-Value = digest.base64_decode(resp.http.X-Metered-Cookie-Value);
    } else {
    ...
    if(resp.http.X-New-Metered-Cookie-Value) {
        header.append(resp.http.Set-Cookie,"p_metered_access=" + digest.base64(resp.http.X-New-Metered-Cookie-Value) + "; path=/; expires=" + resp.http.X-Timestamp + ";");
    }
