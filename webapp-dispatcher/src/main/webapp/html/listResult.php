<div class="large-12 column">
    <div class="generalArgument">
        <h2 class="titleTwo">Result</h2>
        <a href="#" title="">
            <h3 class="titleResult">Notizie di Ieri</h3> 
        </a>
    </div>
</div>
<!-- ARGUMENT LIST -->
<div class="large-12 medium-12 column noLeft">
    <!-- Argument One-->
    <div class="argumentContent">
        <a href="#" title="">Meridiana, azienda convoca i sindacati per confronto su piano di rilancio</a>
        <p>urlan: "È necessario che in questo Paese ci sia una vera riforma del fisco e dato che il Governo ha ritenuto di non confrontarsi col sindacato, chiediamo una forte alleanza ai cittadini perchè ci vuole un grande patto sociale che metta al centro sviluppo e crescita"</p>
    </div>
    <!-- Argument Two-->
    <div class="argumentContent">
        <a href="#" title="">Meridiana, azienda convoca i sindacati per confronto su piano di rilancio</a>
        <p>urlan: "È necessario che in questo Paese ci sia una vera riforma del fisco e dato che il Governo ha ritenuto di non confrontarsi col sindacato, chiediamo una forte alleanza ai cittadini perchè ci vuole un grande patto sociale che metta al centro sviluppo e crescita"</p>
    </div>
    <!-- Argument Three-->
    <div class="argumentContent">
        <a href="#" title="">Meridiana, azienda convoca i sindacati per confronto su piano di rilancio</a>
        <p>urlan: "È necessario che in questo Paese ci sia una vera riforma del fisco e dato che il Governo ha ritenuto di non confrontarsi col sindacato, chiediamo una forte alleanza ai cittadini perchè ci vuole un grande patto sociale che metta al centro sviluppo e crescita"</p>
    </div>
    <!-- Argument Four-->
    <div class="argumentContent">
        <a href="#" title="">Meridiana, azienda convoca i sindacati per confronto su piano di rilancio</a>
        <p>urlan: "È necessario che in questo Paese ci sia una vera riforma del fisco e dato che il Governo ha ritenuto di non confrontarsi col sindacato, chiediamo una forte alleanza ai cittadini perchè ci vuole un grande patto sociale che metta al centro sviluppo e crescita"</p>
    </div>
</div>    
<div class="large-12 column noLeft">
    <div class="contActionList">
        <a class="print" href="#" title="Stampa Pagina"><img src="/img/icon/print.png" alt="Stampa Pagina"> Stampa Pagina</a>
        <a class="upPage" href="#" title="Vai Inizio Pagina"><img src="/img/icon/up.png" alt="Icona Inizio Pagina">Inizio Pagina</a>
    </div>
    <!-- SHARE ON SOCIALS-->
    <div class="interact">
        <ul>
            <li><a href="#" title="Email"><img src="/img/icon/email.png" alt="Email Icon"/></a></li>
            <li><a href="#" title="Facebook"><img src="/img/icon/fbBit.png" alt="Facebook Icon"/></a></li>
            <li><a href="#" title="Twitter"><img src="/img/icon/twitterBit.png" alt="Twitter Icon"/></a></li>
            <li><a href="#" title="Google Plus"><img src="/img/icon/googlePlusBit.png" alt="Google Plus Icon"/></a></li>
            <li><a href="#" title="Pinterest"><img src="/img/icon/pinterestBit.png" alt="Pinterest Icon"/></a></li>
        </ul>
    </div>
    <!-- Comment Article-->
    <div class="commentArticle">
        <a href="#" title="Commenta"><img src="/img/icon/comment.png" alt="Commenta Icona"/></a>
    </div>
    <!-- Cancel the floating css rule -->
    <div class="clear"></div>
</div>
